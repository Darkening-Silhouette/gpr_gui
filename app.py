...
# --- multi time-slice figure patch: Schleitheim/MALA ---

def _multi_ts_save_hd_schleitheim(self):
    try:
        from pathlib import Path
        import datetime
        canvas = self.time_canvas
        out_dir = Path(getattr(getattr(self, 'main', None), 'root', Path('.')))
        out_dir.mkdir(parents=True, exist_ok=True)
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        png = out_dir / f"schleitheim_multislice_time_slice_map_{ts}.png"
        pdf = out_dir / f"schleitheim_multislice_time_slice_map_{ts}.pdf"
        ...
        status = getattr(getattr(self, 'main', None), 'status', None)
...
def _multi_ts_install_schleitheim(self):
    try:
        ...
        try:
            base = float(self.slice_time.value())
        except Exception:
            base = 80.0
        ...
        for i in range(10):
            ...
            sp.setDecimals(2)
            sp.setSingleStep(2.5)
            sp.setSuffix(" ns")
            sp.setValue([50.0, 80.0, 110.0, 140.0, 65.0, 95.0, 125.0, 155.0, 35.0, 170.0][i])
        ...
        self.multi_ts_save_hd.clicked.connect(lambda *_: _multi_ts_save_hd_schleitheim(self))
        ...
        try:
            self.slice_time.valueChanged.connect(lambda v: self.time_slice_values[0][1].setValue(float(v)))
        except Exception:
            pass
        refresh()
        ...
        print("Schleitheim multi time-slice controls not added:", e)


def _multi_ts_plot_schleitheim(self):
    import math
    import numpy as np

    if not hasattr(self, "time_slice_count") or not hasattr(self, "time_slice_values"):
        return _multi_ts_old_plot_schleitheim(self)
    ...
    for tv in times:
        x, y, val = self.collect_slice_points(float(tv))
    ...
    try:
        ...
        pct = 99.0
    amp = float(np.nanpercentile(np.abs(allv[np.isfinite(allv)]), pct))
        ...
        if len(val) >= 4:
            try:
                ...
                last_im = ax.imshow(
                    ...
                    aspect="equal", cmap=self.main.cmap.currentText(),
                    vmin=-local_amp, vmax=local_amp
                )
                ax.scatter(x, y, s=0.6, c="k", alpha=0.06)
            except Exception:
                last_im = ax.scatter(x, y, c=val, s=3, cmap=self.main.cmap.currentText(), vmin=-local_amp, vmax=local_amp)
                ax.set_aspect("equal", adjustable="box")
        else:
    ...
    canvas.draw_idle()
    try:
        self.main.status.setText("Updated multi time-slice map: " + ", ".join(f"{t:.1f} ns" for t in times))
    except Exception:
        pass


try:
    if not hasattr(GPR3DStandardAnalysisTab, "_multi_ts_old_init"):
        GPR3DStandardAnalysisTab._multi_ts_old_init = GPR3DStandardAnalysisTab.__init__
        def _multi_ts_new_init_sch(self, *args, **kwargs):
            GPR3DStandardAnalysisTab._multi_ts_old_init(self, *args, **kwargs)
            _multi_ts_install_schleitheim(self)
        GPR3DStandardAnalysisTab.__init__ = _multi_ts_new_init_sch

    if "_multi_ts_old_plot_schleitheim" not in globals():
        _multi_ts_old_plot_schleitheim = GPR3DStandardAnalysisTab.plot_time_slice

    GPR3DStandardAnalysisTab.plot_time_slice = _multi_ts_plot_schleitheim
except Exception as e:
    print("Schleitheim multi time-slice patch failed:", e)
# --- end multi time-slice figure patch: Schleitheim/MALA ---


# --- Radargram horizontal display width selector patch ---
def _gpr_line_length_m(line):
    try:
        if line.dist is not None and len(line.dist):
            return float(abs(line.dist[-1] - line.dist[0]))
    except Exception:
        pass
    try:
        return float(getattr(line, "traces", 0))
    except Exception:
        return 0.0


def _gpr_radargram_width_px(self):
    # Display-only canvas width selector. This fixes short crosslines being
    # visually stretched to the same width as long ~50 m inlines.
    line_len = _gpr_line_length_m(self.line)
    mode = "Auto by line length"
    try:
        mode = self.main.radargram_width_mode.currentText()
    except Exception:
        pass
    ...
    # Auto by line length. Crosslines around 4 m stay visually compact;
    # long inlines keep enough horizontal space for reflection geometry.
    if line_len <= 6.0:
        return 760
    if line_len <= 12.0:
        return 920
...
def _gpr_apply_radargram_width(self):
    try:
        width = int(_gpr_radargram_width_px(self))
        for canvas in [getattr(self, "raw_canvas", None), getattr(self, "proc_canvas", None)]:
            if canvas is None:
                continue
            canvas.setMinimumWidth(width)
            canvas.setMaximumWidth(width)
            try:
                canvas.fig.set_size_inches(max(width / 110.0, 6.0), 3.7, forward=True)
            except Exception:
                pass
        try:
            self.updateGeometry()
        ...
        print("Radargram width apply failed:", e)


def _gpr_apply_width_to_all_tabs(main_window):
    try:
        for tab in getattr(main_window, "line_tabs", {}).values():
            try:
                _gpr_apply_radargram_width(tab)
            except Exception:
                pass
        try:
            main_window.status.setText("Radargram horizontal display width updated.")
        except Exception:
            pass
    except Exception as e:
        print("Apply radargram width to tabs failed:", e)


try:
    if not hasattr(LineTab, "_old_plot_raw_for_width_selector"):
        LineTab._old_plot_raw_for_width_selector = LineTab.plot_raw
        def _plot_raw_with_width_selector(self, *args, **kwargs):
            out = LineTab._old_plot_raw_for_width_selector(self, *args, **kwargs)
            _gpr_apply_radargram_width(self)
            return out
        LineTab.plot_raw = _plot_raw_with_width_selector

    if not hasattr(LineTab, "_old_plot_processed_for_width_selector"):
        LineTab._old_plot_processed_for_width_selector = LineTab.plot_processed
        def _plot_processed_with_width_selector(self, *args, **kwargs):
            out = LineTab._old_plot_processed_for_width_selector(self, *args, **kwargs)
            _gpr_apply_radargram_width(self)
            return out
        LineTab.plot_processed = _plot_processed_with_width_selector

    if not hasattr(MainWindow, "_old_init_for_width_selector"):
        MainWindow._old_init_for_width_selector = MainWindow.__init__
        def _main_init_with_width_selector(self, *args, **kwargs):
            MainWindow._old_init_for_width_selector(self, *args, **kwargs)

            try:
                if not hasattr(self, "radargram_width_mode"):
                    self.radargram_width_mode = QComboBox()
                    self.radargram_width_mode.addItems([
                        "Auto by line length",
                        "Compact",
                        "Normal",
                        "Wide",
                    ...
                    self.radargram_width_mode.setCurrentText("Auto by line length")
                    self.radargram_width_mode.setToolTip(
                        "Display-only radargram canvas width. "
                        "Use Auto by line length to keep short crosslines compact and long inlines wide."
                    )

                layout = self.processing_controls.layout()
                if layout is not None and not getattr(self, "_radargram_width_control_added", False):
                    row = 4
                    layout.addWidget(QLabel("Radargram width"), row, 0)
                    layout.addWidget(self.radargram_width_mode, row, 1, 1, 2)
                    self._radargram_width_control_added = True

                try:
                    self.radargram_width_mode.currentTextChanged.connect(lambda *_: _gpr_apply_width_to_all_tabs(self))
                    self.radargram_width_mode.currentTextChanged.connect(lambda *_: self.redraw_all())
                except Exception:
                    pass

                _gpr_apply_width_to_all_tabs(self)
            except Exception as e:
                print("Could not add radargram width selector:", e)

        MainWindow.__init__ = _main_init_with_width_selector

    if not hasattr(MainWindow, "_old_load_project_for_width_selector"):
        MainWindow._old_load_project_for_width_selector = MainWindow.load_project
        def _load_project_with_width_selector(self, *args, **kwargs):
            out = MainWindow._old_load_project_for_width_selector(self, *args, **kwargs)
            try:
                _gpr_apply_width_to_all_tabs(self)
            except Exception:
                pass
            return out
        MainWindow.load_project = _load_project_with_width_selector

except Exception as e:
    print("Radargram horizontal display width selector patch failed:", e)
# --- end radargram horizontal display width selector patch ---


# --- Save current radargram view patch ---
def _gpr_active_line_tab(main_window):
    try:
        current = main_window.tabs.currentWidget()
        if isinstance(current, LineTab):
            return current
    except Exception:
        pass

    try:
        title = main_window.tabs.tabText(main_window.tabs.currentIndex()).lower()
    except Exception:
        title = ""

    try:
        if title.startswith("inline") and hasattr(main_window, "inline_tabs"):
            tab = main_window.inline_tabs.currentWidget()
            if isinstance(tab, LineTab):
                return tab
        if title.startswith("crossline") and hasattr(main_window, "crossline_tabs"):
            tab = main_window.crossline_tabs.currentWidget()
            if isinstance(tab, LineTab):
                return tab
    except Exception:
        pass
...
def _gpr_save_current_radargram_view(main_window):
    try:
        from pathlib import Path
        import datetime
        ...
        tab = _gpr_active_line_tab(main_window)
        if tab is None:
            raise RuntimeError("Open an inline or crossline radargram tab first.")

        out_dir = Path(getattr(main_window, "root", Path("."))) / "exports" / "current_views"
        out_dir.mkdir(parents=True, exist_ok=True)

        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        line_name = _gpr_safe_name(getattr(getattr(tab, "line", None), "name", "radargram"))
        ...
        # Save clean matplotlib figures. These preserve current x/y axis limits.
        try:
            tab.raw_canvas.fig.savefig(raw_png, dpi=250, bbox_inches="tight", facecolor="white")
        except Exception as e:
            print("Could not save raw current-view figure:", e)

        try:
            tab.proc_canvas.fig.savefig(proc_png, dpi=250, bbox_inches="tight", facecolor="white")
        except Exception as e:
            print("Could not save processed current-view figure:", e)

        # Save literal current canvas view as one combined image: raw above processed.
        raw_pm = tab.raw_canvas.grab()
        proc_pm = tab.proc_canvas.grab()

        width = max(raw_pm.width(), proc_pm.width())
        height = raw_pm.height() + proc_pm.height()
        img = QImage(width, height, QImage.Format.Format_ARGB32)
        img.fill(Qt.GlobalColor.white)

        painter = QPainter(img)
        painter.drawPixmap(0, 0, raw_pm)
        painter.drawPixmap(0, raw_pm.height(), proc_pm)
        painter.end()
        ...
        try:
            main_window.status.setText(f"Saved current radargram view: {combined_png}")
        except Exception:
            pass

        print(f"Saved current radargram view: {combined_png}")
        print(f"Saved raw: {raw_png}")
        print(f"Saved processed: {proc_png}")

    except Exception as e:
        try:
            from PyQt6.QtWidgets import QMessageBox
            QMessageBox.critical(main_window, "Save current view failed", str(e))
        except Exception:
            print("Save current view failed:", e)


def _gpr_add_save_current_view_button(main_window):
    try:
        if getattr(main_window, "_save_current_view_button_added", False):
            return

        main_window.btn_save_current_view = QPushButton("Save current view")
        main_window.btn_save_current_view.setToolTip(
            "Save the currently displayed raw + processed radargram view. "
            "The saved image uses the current display width, zoom and time window."
        )
        main_window.btn_save_current_view.clicked.connect(lambda *_: _gpr_save_current_radargram_view(main_window))

        layout = main_window.processing_controls.layout()
        if layout is not None:
            # Same new row as the radargram-width selector, but to the right.
            layout.addWidget(main_window.btn_save_current_view, 4, 3, 1, 3)

        main_window._save_current_view_button_added = True
    except Exception as e:
        print("Could not add Save current view button:", e)


try:
    if not hasattr(MainWindow, "_old_init_for_save_current_view"):
        MainWindow._old_init_for_save_current_view = MainWindow.__init__
        def _main_init_with_save_current_view(self, *args, **kwargs):
            MainWindow._old_init_for_save_current_view(self, *args, **kwargs)
            _gpr_add_save_current_view_button(self)
        MainWindow.__init__ = _main_init_with_save_current_view

    if not hasattr(MainWindow, "_old_load_project_for_save_current_view"):
        MainWindow._old_load_project_for_save_current_view = MainWindow.load_project
        def _load_project_with_save_current_view(self, *args, **kwargs):
            out = MainWindow._old_load_project_for_save_current_view(self, *args, **kwargs)
            try:
                _gpr_add_save_current_view_button(self)
            except Exception:
                pass
            return out
        MainWindow.load_project = _load_project_with_save_current_view

except Exception as e:
    print("Save current radargram view patch failed:", e)
# --- end save current radargram view patch ---


# --- Radargram side-by-side fit-to-screen patch ---
def _gpr_set_combo_fit_default(main_window):
    try:
        combo = getattr(main_window, "radargram_width_mode", None)
        if combo is None:
            return
        if combo.findText("Fit to screen") < 0:
            combo.insertItem(0, "Fit to screen")
        combo.setCurrentText("Fit to screen")
    except Exception as e:
        print("Could not set radargram fit default:", e)


def _gpr_make_line_tab_side_by_side(tab):
    try:
        if getattr(tab, "_radargram_side_by_side_done", False):
            return

        from PyQt6.QtWidgets import QWidget, QHBoxLayout, QSizePolicy

        raw_box = tab.raw_canvas.parentWidget()
        proc_box = tab.proc_canvas.parentWidget()
        if raw_box is None or proc_box is None or raw_box is proc_box:
            return
        ...
        old_layout.removeWidget(raw_box)
        old_layout.removeWidget(proc_box)

        side = QWidget(tab)
        side_layout = QHBoxLayout(side)
        side_layout.setContentsMargins(0, 0, 0, 0)
        side_layout.setSpacing(8)
        side_layout.addWidget(raw_box, 1)
        side_layout.addWidget(proc_box, 1)

        raw_box.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        proc_box.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        tab.raw_canvas.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        tab.proc_canvas.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)

        old_layout.addWidget(side, 1)

        tab._radargram_side_by_side_container = side
        tab._radargram_side_by_side_done = True
    except Exception as e:
        print("Could not arrange radargrams side-by-side:", e)
    ...
    try:
        ...
        if mode == "Fit to screen":
            for canvas in [getattr(self, "raw_canvas", None), getattr(self, "proc_canvas", None)]:
                ...
                try:
                    canvas.fig.set_size_inches(7.6, 3.8, forward=True)
                except Exception:
                    pass
            try:
                self.updateGeometry()
        ...
        # Manual modes keep fixed widths; useful for saved/zoomed review.
        line_len = _gpr_line_length_m(self.line) if "_gpr_line_length_m" in globals() else 0.0
        if mode == "Compact":
            width = 760
        elif mode == "Normal":
            width = 1150
        ...
        for canvas in [getattr(self, "raw_canvas", None), getattr(self, "proc_canvas", None)]:
            ...
            canvas.setMinimumHeight(260)
            try:
                canvas.fig.set_size_inches(max(width / 110.0, 6.0), 3.8, forward=True)
            except Exception:
                pass
        try:
            self.updateGeometry()
        ...
        print("Radargram fit/width apply failed:", e)


def _gpr_apply_fit_to_all_tabs(main_window):
    try:
        _gpr_set_combo_fit_default(main_window)
    except Exception:
        pass

    try:
        tabs = []
        if hasattr(main_window, "line_tabs"):
            tabs.extend(list(main_window.line_tabs.values()))
        if hasattr(main_window, "inline_tabs"):
            for i in range(main_window.inline_tabs.count()):
                tabs.append(main_window.inline_tabs.widget(i))
        if hasattr(main_window, "crossline_tabs"):
            for i in range(main_window.crossline_tabs.count()):
                tabs.append(main_window.crossline_tabs.widget(i))
        ...
        for tab in tabs:
            ...
            seen.add(id(tab))
            if isinstance(tab, LineTab):
                _gpr_make_line_tab_side_by_side(tab)
                _gpr_apply_radargram_width(tab)

        try:
            main_window.status.setText("Radargrams set to side-by-side fit-to-screen view.")
        except Exception:
            pass
    except Exception as e:
        print("Apply side-by-side fit to tabs failed:", e)


try:
    if not hasattr(LineTab, "_old_init_for_side_by_side_fit"):
        LineTab._old_init_for_side_by_side_fit = LineTab.__init__
        def _line_init_side_by_side_fit(self, *args, **kwargs):
            LineTab._old_init_for_side_by_side_fit(self, *args, **kwargs)
            _gpr_make_line_tab_side_by_side(self)
            _gpr_apply_radargram_width(self)
        LineTab.__init__ = _line_init_side_by_side_fit

    if not hasattr(MainWindow, "_old_init_for_side_by_side_fit"):
        MainWindow._old_init_for_side_by_side_fit = MainWindow.__init__
        def _main_init_side_by_side_fit(self, *args, **kwargs):
            MainWindow._old_init_for_side_by_side_fit(self, *args, **kwargs)
            try:
                _gpr_set_combo_fit_default(self)
                if hasattr(self, "radargram_width_mode"):
                    try:
                        self.radargram_width_mode.currentTextChanged.connect(lambda *_: _gpr_apply_fit_to_all_tabs(self))
                    except Exception:
                        pass
                _gpr_apply_fit_to_all_tabs(self)
            except Exception as e:
                print("Could not initialise side-by-side radargram fit:", e)
        MainWindow.__init__ = _main_init_side_by_side_fit

    if not hasattr(MainWindow, "_old_load_project_for_side_by_side_fit"):
        MainWindow._old_load_project_for_side_by_side_fit = MainWindow.load_project
        def _load_project_side_by_side_fit(self, *args, **kwargs):
            out = MainWindow._old_load_project_for_side_by_side_fit(self, *args, **kwargs)
            try:
                _gpr_apply_fit_to_all_tabs(self)
            except Exception:
                pass
            return out
        MainWindow.load_project = _load_project_side_by_side_fit

except Exception as e:
    print("Radargram side-by-side fit-to-screen patch failed:", e)
# --- end radargram side-by-side fit-to-screen patch ---


# --- Compact default radargram width patch ---
def _gpr_force_compact_default(main_window):
    try:
        combo = getattr(main_window, "radargram_width_mode", None)
        if combo is not None:
            if combo.findText("Compact") >= 0:
                combo.setCurrentText("Compact")
        if "_gpr_apply_fit_to_all_tabs" in globals():
            _gpr_apply_fit_to_all_tabs(main_window)
        elif "_gpr_apply_width_to_all_tabs" in globals():
            _gpr_apply_width_to_all_tabs(main_window)
    except Exception as e:
        print("Could not force Schleitheim compact radargram default:", e)


try:
    if not hasattr(MainWindow, "_old_init_for_compact_default"):
        MainWindow._old_init_for_compact_default = MainWindow.__init__
        def _main_init_compact_default(self, *args, **kwargs):
            MainWindow._old_init_for_compact_default(self, *args, **kwargs)
            _gpr_force_compact_default(self)
        MainWindow.__init__ = _main_init_compact_default

    if not hasattr(MainWindow, "_old_load_project_for_compact_default"):
        MainWindow._old_load_project_for_compact_default = MainWindow.load_project
        def _load_project_compact_default(self, *args, **kwargs):
            out = MainWindow._old_load_project_for_compact_default(self, *args, **kwargs)
            _gpr_force_compact_default(self)
            return out
        MainWindow.load_project = _load_project_compact_default
except Exception as e:
    print("Compact default radargram width patch failed:", e)
# --- end compact default radargram width patch ---


# --- Ringing suppression + NMO correction features for Schleitheim/MALA ---
def _rs_time_axis(ns, dt_ns):
    return np.arange(int(ns), dtype=float) * float(dt_ns)


def _rs_early_mute_taper(data, dt_ns, mute_end_ns, taper_ns):
    import numpy as np
    out = np.asarray(data, dtype=np.float64).copy()
    if out.ndim != 2 or dt_ns <= 0 or mute_end_ns <= 0:
        return out

    ns = out.shape[1]
    t = _rs_time_axis(ns, dt_ns)
    mute_end = float(mute_end_ns)
    taper = max(0.0, float(taper_ns))

    w = np.ones(ns, dtype=float)
    if taper <= 0:
        w[t < mute_end] = 0.0
    else:
...
def _rs_predictive_decon(data, dt_ns, lag_ns, op_ns=30.0, prewhite_pct=0.5):
    import numpy as np
    x = np.asarray(data, dtype=np.float64)
    if x.ndim != 2 or dt_ns <= 0:
        return x.copy()

    lag = max(1, int(round(float(lag_ns) / max(dt_ns, 1e-9))))
    op = max(lag + 1, int(round(float(op_ns) / max(dt_ns, 1e-9))))
    ns = x.shape[1]
    if lag >= ns - 2:
        return x.copy()

    # Conservative single-lag predictive subtraction with local autocorrelation.
    # This attenuates repeated ringing while avoiding aggressive wavelet shaping.
    out = x.copy()
    end = min(ns, lag + op)
    a = x[:, lag:end]
    b = x[:, :end-lag]
...
def _rs_nmo_zero_offset(data, dt_ns, offset_m, velocity_m_ns):
    import numpy as np
    x = np.asarray(data, dtype=np.float64)
    if x.ndim != 2 or dt_ns <= 0 or offset_m <= 0 or velocity_m_ns <= 0:
        return x.copy()

    ns = x.shape[1]
    tau = _rs_time_axis(ns, dt_ns)  # desired zero-offset time
    t_offset = float(offset_m) / max(float(velocity_m_ns), 1e-9)
    t_meas = np.sqrt(tau * tau + t_offset * t_offset)

    out = np.empty_like(x)
    src_t = _rs_time_axis(ns, dt_ns)
    for i in range(x.shape[0]):
        out[i, :] = np.interp(t_meas, src_t, x[i, :], left=0.0, right=0.0)
    return out


def _rs_tpower_gain(data, dt_ns, sec_power):
    import numpy as np
    out = np.asarray(data, dtype=np.float64).copy()
    if out.ndim != 2 or dt_ns <= 0 or sec_power <= 0:
        return out
    t = _rs_time_axis(out.shape[1], dt_ns)
    tmax_gain = 30.0
    tt = np.minimum(np.maximum(t, 0.0), tmax_gain)
    gain = (1.0 + tt / max(tmax_gain, 1e-9)) ** float(sec_power)
    gain /= max(float(gain[0]), 1e-12)
...
def _rs_agc_gain(data, dt_ns, window_ns):
    import numpy as np
    x = np.asarray(data, dtype=np.float64)
    if x.ndim != 2 or dt_ns <= 0 or window_ns <= 0 or x.shape[1] < 3:
        return x.copy()
    ns = x.shape[1]
    w = max(3, int(round(float(window_ns) / max(dt_ns, 1e-9))))
    w = min(w, ns)
    if w % 2 == 0:
        w -= 1
    if w < 3:
...
def _mala_get_extra_settings(main):
    return {
        "mute_on": bool(getattr(main, "ring_mute_enabled", _GPRSecOff()).isChecked()),
        "mute_end": float(getattr(main, "ring_mute_end_ns", main.display_tmax).value()) if hasattr(main, "ring_mute_end_ns") else 60.0,
        "mute_taper": float(getattr(main, "ring_mute_taper_ns", main.display_tmax).value()) if hasattr(main, "ring_mute_taper_ns") else 10.0,
        "nmo_on": bool(getattr(main, "nmo_enabled", _GPRSecOff()).isChecked()),
        "offset": float(getattr(main, "nmo_offset_m", main.display_tmax).value()) if hasattr(main, "nmo_offset_m") else 0.33,
        "velocity": float(getattr(main, "nmo_velocity_m_ns", main.display_tmax).value()) if hasattr(main, "nmo_velocity_m_ns") else 0.08,
        "decon_on": bool(getattr(main, "pred_decon_enabled", _GPRSecOff()).isChecked()),
        "decon_lag": float(getattr(main, "pred_decon_lag_ns", main.display_tmax).value()) if hasattr(main, "pred_decon_lag_ns") else 8.0,
        "decon_op": float(getattr(main, "pred_decon_op_ns", main.display_tmax).value()) if hasattr(main, "pred_decon_op_ns") else 30.0,
        "decon_white": float(getattr(main, "pred_decon_white_pct", main.display_tmax).value()) if hasattr(main, "pred_decon_white_pct") else 0.5,
    }


def _mala_apply_ringing_nmo(main, arr, dt_ns):
    s = _mala_get_extra_settings(main)
    out = np.asarray(arr, dtype=np.float64)

    # Ringing suppression before gain.
    if s["mute_on"]:
        out = _rs_early_mute_taper(out, dt_ns, s["mute_end"], s["mute_taper"])
    if s["decon_on"]:
        out = _rs_predictive_decon(out, dt_ns, s["decon_lag"], s["decon_op"], s["decon_white"])
    if s["nmo_on"]:
        out = _rs_nmo_zero_offset(out, dt_ns, s["offset"], s["velocity"])
    return out


def _mala_processing_label(main, sec_on, sec_val):
    parts = ["Processed: DC removal"]
    if float(main.dewow_ns.value()) > 0:
        parts.append("dewow")
    if main.bg.isChecked():
        parts.append("background removal")
    if main.bp.isChecked():
        parts.append("bandpass")
    if getattr(main, "ring_mute_enabled", None) is not None and main.ring_mute_enabled.isChecked():
        parts.append(f"early mute/taper ({main.ring_mute_end_ns.value():.0f} ns)")
    if getattr(main, "pred_decon_enabled", None) is not None and main.pred_decon_enabled.isChecked():
        parts.append("predictive decon")
    if getattr(main, "nmo_enabled", None) is not None and main.nmo_enabled.isChecked():
        parts.append(f"NMO zero-offset ({main.nmo_offset_m.value():.2f} m)")
    if sec_on and sec_val > 0:
        parts.append(f"T-power gain ({sec_val:.2f})")
    if main.agc.isChecked():
        parts.append("AGC gain")
    return " + ".join(parts)


def _mala_process_with_ringing_nmo(main, line, raw):
    sec_on = bool(getattr(main, "sec_enabled", _GPRSecOff()).isChecked())
    sec_val = float(main.sec_power.value()) if sec_on else 0.0
    line.processing_label = _mala_processing_label(main, sec_on, sec_val)

    # Base processing without gain first, so ringing suppression happens before gain/AGC.
    out = process_gpr(
        raw,
        dt_ns=line.dt_ns,
        dewow_ns=float(main.dewow_ns.value()),
        remove_background=main.bg.isChecked(),
        use_bandpass=main.bp.isChecked(),
        low_mhz=float(main.low_mhz.value()),
        high_mhz=float(main.high_mhz.value()),
        sec_power=0.0,
        background_window_traces=int(main.bg_window.value()),
        use_agc=False,
        agc_window_ns=float(main.agc_window.value()),
    )

    out = _mala_apply_ringing_nmo(main, out, line.dt_ns)

    if sec_val > 0:
        out = _rs_tpower_gain(out, line.dt_ns, sec_val)
    if main.agc.isChecked():
        out = _rs_agc_gain(out, line.dt_ns, float(main.agc_window.value()))
    return out


def _mala_clear_processed_cache(main):
    try:
        for ln in getattr(main, "lines", []):
            ln.processed = None
        if hasattr(main, "status"):
            main.status.setText("Processing settings changed; processed cache cleared.")
    except Exception:
        pass


def _mala_add_ringing_nmo_controls(main):
    try:
        if getattr(main, "_ringing_nmo_controls_added", False):
            return
        ...
        main.ring_mute_enabled = QCheckBox("Ringing mute/taper")
        main.ring_mute_enabled.setChecked(True)
        main.ring_mute_end_ns = dspin(60.0, 0.0, 1000.0, 5.0, " ns")
        main.ring_mute_taper_ns = dspin(10.0, 0.0, 200.0, 1.0, " ns")

        main.nmo_enabled = QCheckBox("NMO zero-offset")
        main.nmo_enabled.setChecked(True)
        main.nmo_offset_m = dspin(0.33, 0.0, 5.0, 0.01, " m")
        main.nmo_velocity_m_ns = dspin(0.08, 0.02, 0.30, 0.005, " m/ns")

        main.pred_decon_enabled = QCheckBox("Predictive decon")
        main.pred_decon_enabled.setChecked(False)
        main.pred_decon_lag_ns = dspin(8.0, 0.1, 100.0, 0.5, " ns")
        main.pred_decon_op_ns = dspin(30.0, 1.0, 300.0, 1.0, " ns")
        main.pred_decon_white_pct = dspin(0.50, 0.0, 10.0, 0.1, " %")

        lay = main.processing_controls.layout()
        if lay is not None:
            row = 8
            lay.addWidget(main.ring_mute_enabled, row, 0)
            lay.addWidget(QLabel("Mute end"), row, 1)
            lay.addWidget(main.ring_mute_end_ns, row, 2)
            lay.addWidget(QLabel("Taper"), row, 3)
            lay.addWidget(main.ring_mute_taper_ns, row, 4)

            row += 1
            lay.addWidget(main.nmo_enabled, row, 0)
            lay.addWidget(QLabel("Offset"), row, 1)
            lay.addWidget(main.nmo_offset_m, row, 2)
            lay.addWidget(QLabel("Velocity"), row, 3)
            lay.addWidget(main.nmo_velocity_m_ns, row, 4)

            row += 1
            lay.addWidget(main.pred_decon_enabled, row, 0)
            lay.addWidget(QLabel("Lag"), row, 1)
            lay.addWidget(main.pred_decon_lag_ns, row, 2)
            lay.addWidget(QLabel("Op"), row, 3)
            lay.addWidget(main.pred_decon_op_ns, row, 4)
            lay.addWidget(QLabel("White"), row, 5)
            lay.addWidget(main.pred_decon_white_pct, row, 6)

        for w in [
            main.ring_mute_enabled, main.nmo_enabled, main.pred_decon_enabled
        ]:
            try:
                w.toggled.connect(lambda *_: _mala_clear_processed_cache(main))
            except Exception:
                pass
        for w in [
            main.ring_mute_end_ns, main.ring_mute_taper_ns,
            main.nmo_offset_m, main.nmo_velocity_m_ns,
            main.pred_decon_lag_ns, main.pred_decon_op_ns, main.pred_decon_white_pct
        ]:
            try:
                w.valueChanged.connect(lambda *_: _mala_clear_processed_cache(main))
            except Exception:
                pass

        main._ringing_nmo_controls_added = True
    except Exception as e:
        print("Could not add Schleitheim ringing/NMO controls:", e)


try:
    if not hasattr(MainWindow, "_old_init_for_ringing_nmo"):
        MainWindow._old_init_for_ringing_nmo = MainWindow.__init__
        def _main_init_ringing_nmo(self, *args, **kwargs):
            MainWindow._old_init_for_ringing_nmo(self, *args, **kwargs)
            _mala_add_ringing_nmo_controls(self)
        MainWindow.__init__ = _main_init_ringing_nmo

    if not hasattr(MainWindow, "_old_process_current_for_ringing_nmo"):
        MainWindow._old_process_current_for_ringing_nmo = MainWindow.process_current_line
        def _main_process_current_ringing_nmo(self, *args, **kwargs):
            tab = self.current_line_tab()
            if tab is None:
                return
            try:
                line = tab.line
                raw = self.load_raw_line(line)
                line.processed = _mala_process_with_ringing_nmo(self, line, raw)
                tab.plot()
                self.status.setText(f"Processed {line.name} with ringing/NMO QC")
            except Exception as e:
                QMessageBox.critical(self, "Processing error", str(e))
        MainWindow.process_current_line = _main_process_current_ringing_nmo

    if "GPR3DAnalysisTab" in globals() and not hasattr(GPR3DAnalysisTab, "_old_ensure_data_for_ringing_nmo"):
        GPR3DAnalysisTab._old_ensure_data_for_ringing_nmo = GPR3DAnalysisTab.ensure_data
        def _gpr3d_ensure_data_ringing_nmo(self, line):
            raw = self.main.load_raw_line(line)
            if self.mode.currentText() == "raw":
                return raw
            if line.processed is None:
                line.processed = _mala_process_with_ringing_nmo(self.main, line, raw)
            return line.processed
        GPR3DAnalysisTab.ensure_data = _gpr3d_ensure_data_ringing_nmo

    if "_gpr3d_fixed_ensure_data" in globals():
        def _gpr3d_fixed_ensure_data(self, line):
            raw = _gpr3d_fixed_orient(self, line, self.main.load_raw_line(line))
            line.processed = _mala_process_with_ringing_nmo(self.main, line, raw)
            line.processed = _gpr3d_fixed_orient(self, line, line.processed)
            return line.processed
        globals()["_gpr3d_fixed_ensure_data"] = _gpr3d_fixed_ensure_data

    print("Ringing suppression + NMO correction controls active for Schleitheim/MALA.")
except Exception as e:
    print("Schleitheim ringing/NMO patch failed:", e)
# --- end ringing suppression + NMO correction features for Schleitheim/MALA ---


# --- Safe default ringing/NMO settings patch for Schleitheim/MALA ---
def _mala_apply_safe_ringing_nmo_defaults(main):
    try:
        # Normal/default interpretation view: keep shallow reflectors visible.
        if hasattr(main, "ring_mute_enabled"):
            main.ring_mute_enabled.setChecked(False)
        if hasattr(main, "ring_mute_end_ns"):
            main.ring_mute_end_ns.setValue(35.0)
        if hasattr(main, "ring_mute_taper_ns"):
            main.ring_mute_taper_ns.setValue(10.0)

        # NMO is safe as default because finite-offset correction is small but methodologically correct.
        if hasattr(main, "nmo_enabled"):
            main.nmo_enabled.setChecked(True)
        if hasattr(main, "nmo_offset_m"):
            main.nmo_offset_m.setValue(0.33)
        if hasattr(main, "nmo_velocity_m_ns"):
            main.nmo_velocity_m_ns.setValue(0.08)

        # Conservative: do not deconvolve unless explicitly testing ringing.
        if hasattr(main, "pred_decon_enabled"):
            main.pred_decon_enabled.setChecked(False)
        if hasattr(main, "pred_decon_lag_ns"):
            main.pred_decon_lag_ns.setValue(8.0)
        if hasattr(main, "pred_decon_op_ns"):
            main.pred_decon_op_ns.setValue(30.0)
        if hasattr(main, "pred_decon_white_pct"):
            main.pred_decon_white_pct.setValue(0.50)

        # Keep visibility similar to the useful previous radargrams.
        if hasattr(main, "sec_enabled"):
            main.sec_enabled.setChecked(True)
        if hasattr(main, "sec_power"):
            main.sec_power.setValue(2.25)
        if hasattr(main, "agc"):
            main.agc.setChecked(True)

        # Full vertical context by default.
        if hasattr(main, "display_tmin"):
            main.display_tmin.setValue(0.0)
        if hasattr(main, "display_tmax"):
            try:
                main.display_tmax.setValue(max(float(main.display_tmax.value()), 180.0))
            except Exception:
                main.display_tmax.setValue(180.0)

        # Clear old processed cache so the new defaults are actually used.
        for ln in getattr(main, "lines", []):
            try:
                ln.processed = None
            except Exception:
                pass
        try:
            main.status.setText("Default processing: shallow visible, NMO ON, ringing mute OFF.")
        except Exception:
            pass
    except Exception as e:
        print("Could not apply Schleitheim safe ringing/NMO defaults:", e)


try:
    if not hasattr(MainWindow, "_old_init_for_safe_ringing_defaults"):
        MainWindow._old_init_for_safe_ringing_defaults = MainWindow.__init__
        def _main_init_safe_ringing_defaults(self, *args, **kwargs):
            MainWindow._old_init_for_safe_ringing_defaults(self, *args, **kwargs)
            _mala_apply_safe_ringing_nmo_defaults(self)
        MainWindow.__init__ = _main_init_safe_ringing_defaults

    if not hasattr(MainWindow, "_old_load_project_for_safe_ringing_defaults"):
        MainWindow._old_load_project_for_safe_ringing_defaults = MainWindow.load_project
        def _load_project_safe_ringing_defaults(self, *args, **kwargs):
            out = MainWindow._old_load_project_for_safe_ringing_defaults(self, *args, **kwargs)
            _mala_apply_safe_ringing_nmo_defaults(self)
            return out
        MainWindow.load_project = _load_project_safe_ringing_defaults

    print("Safe default ringing/NMO settings active for Schleitheim/MALA.")
except Exception as e:
    print("Safe default ringing/NMO settings patch failed for Schleitheim:", e)
# --- end safe default ringing/NMO settings patch for Schleitheim/MALA ---


# --- Radargram title + width-combo persistence fix ---
def _gpr_short_line_name(line):
    try:
        from pathlib import Path as _Path
        return _Path(str(getattr(line, "name", "line"))).name
    except Exception:
        return str(getattr(line, "name", "line"))


def _gpr_short_proc_parts(main):
    parts = ["DC"]
    try:
        if float(main.dewow_ns.value()) > 0:
            parts.append("dewow")
    ...
    try:
        if getattr(main, "ring_mute_enabled", None) is not None and main.ring_mute_enabled.isChecked():
            parts.append(f"mute {main.ring_mute_end_ns.value():.0f} ns")
    except Exception:
        pass
    try:
        if getattr(main, "nmo_enabled", None) is not None and main.nmo_enabled.isChecked():
    ...
    try:
        ...
        pass
    try:
        if getattr(main, "sec_enabled", None) is not None and main.sec_enabled.isChecked() and float(main.sec_power.value()) > 0:
...
# The older side-by-side patch forced "Fit to screen" every time it reapplied layout.
# Override that behaviour: set Compact once at startup, then preserve user selection.
def _gpr_set_combo_fit_default(main_window):
    try:
        combo = getattr(main_window, "radargram_width_mode", None)
        if combo is None:
        ...
        if combo.findText("Fit to screen") < 0:
            combo.insertItem(0, "Fit to screen")
        if combo.findText("Compact") >= 0 and not getattr(main_window, "_radargram_width_defaulted_once", False):
            combo.setCurrentText("Compact")
            main_window._radargram_width_defaulted_once = True
    except Exception as e:
        print("Could not preserve radargram width combo:", e)


def _gpr_fix_sch_title_after_plot(tab, which):
    try:
        line_name = _gpr_short_line_name(tab.line)
        if which == "raw":
            ax = getattr(tab, "raw_ax", None)
            canvas = getattr(tab, "raw_canvas", None)
            title = f"{line_name} — raw"
        else:
            ax = getattr(tab, "proc_ax", None)
            canvas = getattr(tab, "proc_canvas", None)
            title = f"{line_name} — processed | {_gpr_short_proc_parts(tab.main)}"
        ...
        try:
            canvas.fig.subplots_adjust(top=0.88)
        except Exception:
            pass
        canvas.draw_idle()
    except Exception as e:
        print("Schleitheim title fix failed:", e)


try:
    if not hasattr(LineTab, "_old_plot_raw_for_title_width_fix"):
        LineTab._old_plot_raw_for_title_width_fix = LineTab.plot_raw
        def _plot_raw_title_width_fix(self, *args, **kwargs):
            out = LineTab._old_plot_raw_for_title_width_fix(self, *args, **kwargs)
            _gpr_fix_sch_title_after_plot(self, "raw")
            return out
        LineTab.plot_raw = _plot_raw_title_width_fix

    if not hasattr(LineTab, "_old_plot_processed_for_title_width_fix"):
        LineTab._old_plot_processed_for_title_width_fix = LineTab.plot_processed
        def _plot_processed_title_width_fix(self, *args, **kwargs):
            out = LineTab._old_plot_processed_for_title_width_fix(self, *args, **kwargs)
            _gpr_fix_sch_title_after_plot(self, "processed")
            return out
        LineTab.plot_processed = _plot_processed_title_width_fix

    print("Schleitheim radargram title and width-combo persistence fix active.")
except Exception as e:
    print("Schleitheim title/width-combo fix patch failed:", e)
# --- end radargram title + width-combo persistence fix ---
