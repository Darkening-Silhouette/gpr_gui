...
# --- Fast 2-D migration line-list patch ---
def _gpr2d_light_line_key(line):
    try:
        parent = line.folder.parent.name.lower()
    except Exception:
        parent = ""
    rank = 0 if parent == "inline" else 1 if parent == "crossline" else 2
    try:
        num = int(getattr(line, "number", 999999))
    except Exception:
        num = 999999
    return (rank, num, str(getattr(line, "name", "")))


def _gpr2d_available_lines_light(self):
    """
    Lightweight line-list provider for the 2-D migration tab.

    Important: do NOT call selected_lines_for_maps() here, because that can
    trigger bad-line QC and process every line during application startup.
    The 2-D migration tab only needs metadata for the combo box; the selected
    line is processed later when the user runs migration.
    """
    lines = []

    if getattr(self, "kind", "") == "schleitheim":
        owner = getattr(self, "owner", None)
        main = getattr(owner, "main", None)
        all_lines = list(getattr(main, "lines", []))

        group = "all lines"
        cross_step = 1
        inline_step = 1
        try:
            group = owner.line_group.currentText()
        except Exception:
            pass
        try:
            cross_step = max(1, int(owner.cross_step.value()))
        except Exception:
            pass
        try:
            inline_step = max(1, int(owner.inline_step.value()))
        except Exception:
            pass
        ...
        for line in all_lines:
            ...
            try:
                num = int(getattr(line, "number", 0))
            except Exception:
                num = 0

            if parent == "inline" and inline_step > 1 and ((num - 1) % inline_step != 0):
                continue
            if parent == "crossline" and cross_step > 1 and ((num - 1) % cross_step != 0) and num not in {1, 201}:
                continue
        ...
        owner = getattr(self, "owner", None)
        try:
            lines = list(owner.selected_lines())
        except Exception:
            lines = list(getattr(getattr(owner, "owner", None), "lines", []))

    return sorted(lines, key=_gpr2d_light_line_key)


try:
    if "GPR2DMigrationTab" in globals() and not getattr(GPR2DMigrationTab, "_fast_line_list_patched", False):
        GPR2DMigrationTab._available_lines = _gpr2d_available_lines_light
        GPR2DMigrationTab._fast_line_list_patched = True
        print("Fast 2-D migration line-list patch active: avoids processing all lines at startup.")
except Exception as e:
    print("Fast 2-D migration line-list patch failed:", e)
# --- end fast 2-D migration line-list patch ---


# --- Schleitheim 3-D migration big X/Y section tabs patch ---
def _g3d_bigsection_add_tabs(obj):
    """Put the existing 2x2 overview plus two large section canvases into tabs."""
    try:
        if getattr(obj, "_g3d_bigsection_tabs_added", False):
            return

        from PyQt6.QtWidgets import QTabWidget, QSizePolicy

        overview_canvas = getattr(obj, "canvas", None)
        if overview_canvas is None:
            return

        parent = overview_canvas.parentWidget()
        if parent is None or parent.layout() is None:
            return

        lay = parent.layout()
        idx = lay.indexOf(overview_canvas)
        if idx < 0:
            return

        lay.removeWidget(overview_canvas)

        tabs = QTabWidget(parent)
        tabs.setDocumentMode(True)

        obj._g3d_overview_canvas = overview_canvas
        obj._g3d_x_canvas = _PVCanvas(13.5, 5.8)
        obj._g3d_y_canvas = _PVCanvas(13.5, 5.8)

        for cv in (obj._g3d_overview_canvas, obj._g3d_x_canvas, obj._g3d_y_canvas):
            try:
                cv.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
                cv.setMinimumHeight(360)
            except Exception:
                pass

        tabs.addTab(obj._g3d_overview_canvas, "Overview 2x2")
        tabs.addTab(obj._g3d_x_canvas, "X section — across survey")
        tabs.addTab(obj._g3d_y_canvas, "Y section — along survey")

        lay.insertWidget(idx, tabs, 1)
        obj._g3d_bigsection_tabs = tabs
        obj._g3d_bigsection_tabs_added = True

        # Preserve old API: overview plotting still uses self.canvas.
        obj.canvas = overview_canvas

    except Exception as e:
        print("Could not add Schleitheim 3-D migration big section tabs:", e)


def _g3d_bigsection_indices(obj, r):
    import numpy as _np
    mig = _np.asarray(r["migrated"])
    nz, nx, ny = mig.shape
    ...
    try:
        if "_g3d_selected_indices_reference_style" in globals():
            ix, iy = _g3d_selected_indices_reference_style(obj, r)
            return max(0, min(ix, nx - 1)), max(0, min(iy, ny - 1))
    except Exception:
        pass

    try:
        if "_g3d_selected_cube_indices" in globals():
            ix, iy, _ = _g3d_selected_cube_indices(obj, r)
            return max(0, min(ix, nx - 1)), max(0, min(iy, ny - 1))
    except Exception:
        pass

    try:
        if hasattr(obj, "inline_section_spin") and obj.inline_section_spin.maximum() > 1:
            ix = int(obj.inline_section_spin.value()) - 1
    except Exception:
        pass

    try:
        if hasattr(obj, "crossline_section_spin") and obj.crossline_section_spin.maximum() > 1:
            iy = int(obj.crossline_section_spin.value()) - 1
    except Exception:
        pass
...
def _g3d_bigsection_clip(obj, a, b):
    import numpy as _np
    pct = 98.5
    try:
        pct = float(obj.clip_pct.value())
    except Exception:
        pass

    vals = _np.concatenate([
        _np.ravel(_np.asarray(a, dtype=float)),
        _np.ravel(_np.asarray(b, dtype=float)),
    ])
    vals = vals[_np.isfinite(vals)]
    if vals.size == 0:
        return 1.0

    v = float(_np.nanpercentile(_np.abs(vals), pct))
    if not _np.isfinite(v) or v <= 0:
        v = float(_np.nanmax(_np.abs(vals)) + 1e-9)
    return max(v, 1e-12)


def _g3d_bigsection_cmap(obj):
    try:
        if "_g3d_section_cmap" in globals():
            return _g3d_section_cmap(obj)
    except Exception:
        pass
    try:
        txt = obj.section_cmap.currentText().lower()
        if "grey" in txt or "gray" in txt:
            return "gray"
    except Exception:
        pass
...
def _g3d_bigsection_plot_one(obj, canvas, input2d, migrated2d, lateral, input_vertical, depth, title_prefix, xlabel):
    import numpy as _np
    ...
    input2d = _np.asarray(input2d, dtype=float)
    migrated2d = _np.asarray(migrated2d, dtype=float)
    lateral = _np.asarray(lateral, dtype=float)
    input_vertical = _np.asarray(input_vertical, dtype=float)
    depth = _np.asarray(depth, dtype=float)

    cmap = _g3d_bigsection_cmap(obj)
    clim = _g3d_bigsection_clip(obj, input2d, migrated2d)

    panels = [
        (axes[0], input2d, input_vertical, "Input before migration", "TWT [ns]"),
        (axes[1], migrated2d, depth, "Migrated", "Depth [m]"),
    ]

    for ax, data2d, vertical, title, ylabel in panels:
        im = ax.imshow(
            data2d,
            aspect="auto",
            origin="upper",
        ...
        ax.set_ylabel(ylabel)
        ax.grid(False)
        fig.colorbar(im, ax=ax, shrink=0.88, label="Amplitude")
...
def _g3d_bigsection_plot_tabs(obj):
    """Draw large X/Y section tabs from obj.last."""
    try:
        import numpy as _np

        if not getattr(obj, "last", None):
            return

        _g3d_bigsection_add_tabs(obj)

        xcv = getattr(obj, "_g3d_x_canvas", None)
        ycv = getattr(obj, "_g3d_y_canvas", None)
        if xcv is None or ycv is None:
            return
        ...
        ix, iy = _g3d_bigsection_indices(obj, r)

        vel = float(r.get("velocity", 0.0))
        dx = float(r.get("dx", 0.0))
        dy = float(r.get("dy", 0.0))

        # X section = fixed along-survey index, section across survey.
        _g3d_bigsection_plot_one(
            obj,
            xcv,
            cube_in[:, ix, :],
            mig[:, ix, :],
            ...
            t,
            depth,
            f"Across-survey vertical section at X={ix+1}/{mig.shape[1]}  x={float(x[ix]):.2f} m | v={vel:.3f} m/ns",
            "Across-survey position [m]",
        )

        # Y section = fixed across-survey index, section along survey.
        _g3d_bigsection_plot_one(
            obj,
            ycv,
            cube_in[:, :, iy],
            ...
            t,
            depth,
            f"Along-survey vertical section at Y={iy+1}/{mig.shape[2]}  y={float(y[iy]):.2f} m | v={vel:.3f} m/ns",
            "Along-survey position [m]",
        )

        try:
            obj._status(
                f"Large 3-D migration section tabs updated: X={ix+1}/{mig.shape[1]}, "
                f"Y={iy+1}/{mig.shape[2]}, v={vel:.3f} m/ns, dx={dx:.3f}, dy={dy:.3f} m."
            )
        except Exception:
            pass

    except Exception as e:
        print("Could not update Schleitheim 3-D migration big section tabs:", e)


def _g3d_bigsection_patch_class(cls):
    if cls is None or getattr(cls, "_g3d_bigsection_tabs_patched", False):
        return

    old_init = cls.__init__
    def new_init(self, *args, _old=old_init, **kwargs):
        _old(self, *args, **kwargs)
        _g3d_bigsection_add_tabs(self)

    cls.__init__ = new_init

    old_plot = cls.plot_result
    def new_plot_result(self, *args, _old=old_plot, **kwargs):
        out = _old(self, *args, **kwargs)
        _g3d_bigsection_plot_tabs(self)
        return out

    cls.plot_result = new_plot_result
    cls._g3d_bigsection_tabs_patched = True
...
try:
    for _name in ("GPR3DMigrationTab", "Professor3DMigrationTab"):
        _cls = globals().get(_name)
        if _cls is not None:
            _g3d_bigsection_patch_class(_cls)
    print("Schleitheim 3-D migration big X/Y section tabs patch active.")
except Exception as e:
    print("Schleitheim 3-D migration big X/Y section tabs patch failed:", e)
# --- end Schleitheim 3-D migration big X/Y section tabs patch ---
